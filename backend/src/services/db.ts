...
const initialLeads: Partial<ILead>[] = [
  {
    name: "David Miller",
...
async function seedMemoryDatabase() {
  ...
  console.log(`Local Resilient Database initialized with ${memoryStore.users.length} users and ${memoryStore.leads.length} leads.`);
}
